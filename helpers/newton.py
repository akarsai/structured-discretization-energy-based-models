...
# - only using jax internals and dense jacobians
# - using the matrix solvers of lineax
#
...
# sparse newtons method
from jax.scipy.sparse.linalg import gmres
from helpers.old.sparse import sparse_frobenius_norm
...
def newton_sparse(
    ...
    :param f: function to find root of (w.r.t. first argument)
    :param Df_sparse: sparse jacobian of f (w.r.t. first argument)
    :param max_iter: maximum number of iterations for newton solver
    :param tol: tolerance for newton solver
    :param debug: debug flag
    :return: callable newton solver
    ...
    def solver_with_stopping_criterion(x0, *args, **kwargs):
        ...
        def body(tup):
            i, x = tup
            update = gmres(Df_sparse(x, *args, **kwargs), f(x, *args, **kwargs), atol=1e-14, tol=1e-14)[0] # [0] because solver returns (x,info)
            return  i+1, x - update
    ...
    def solver_without_stopping_criterion( x0, * args, ** kwargs  ):
        ...
        def body(i, x):
            ff = f(x, *args, **kwargs)
            Dff_sparse = Df_sparse(x, *args, **kwargs)
            ...
            def false_fun():
                return gmres(Dff_sparse, ff, atol=1e-14, tol=1e-14)[0] # [0] because solver returns (x,info)

            update = jax.lax.cond(jnp.isnan(sparse_frobenius_norm(Dff_sparse)), true_fun, false_fun)
